...
public class ItemController {
    ...
    public String itemEditGet(Model model, Item item) throws IOException {
        ItemCategory itemCategory = new ItemCategory();
        itemCategory.setStart(0);
        itemCategory.setEnd(Integer.MAX_VALUE);
        List<ItemCategory> itemCategoryList = itemCategoryMapper.list(itemCategory);
        ...
        if (item.getId() != 0) {
            ...
            String mongodbid = item1.getMongodbid();
            imageName = mongodbOper.FindFile(mongodbid,sb.toString());
...
//                mongoUtil.uploadFile(tempFile, id, metaMap);
                String bl =mongodbOper.SaveFile(tempFile,file.getOriginalFilename());
                if (bl != "") {
                    tempFile.delete();
                    item.setMongodbid(bl);
                }
//                getFile.delete();
            } catch (Exception ex) {
                ex.printStackTrace();
